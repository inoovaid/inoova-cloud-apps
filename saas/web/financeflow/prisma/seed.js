...
async function main() {
  ...
  const clients = await Promise.all([
    ...
    db.client.create({ data: { name: 'Startup Inova',            email: 'contato@startuinova.com',    phone: '(51) 95555-7890', cpfCnpj: '56.789.012/0001-34', city: 'Porto Alegre',   state: 'RS', tags: 'empresa,startup',     tenantId: tenant.id } }),
  ...
  const accountPlans = await Promise.all([
    db.accountPlan.create({ data: { name: 'Receita de Serviços', type: 'receita', tenantId: tenant.id } }), // 0
    db.accountPlan.create({ data: { name: 'Vendas de Produtos',  type: 'receita', tenantId: tenant.id } }), // 1
    db.accountPlan.create({ data: { name: 'Aluguel',             type: 'despesa', tenantId: tenant.id } }), // 2
    db.accountPlan.create({ data: { name: 'Salários',            type: 'despesa', tenantId: tenant.id } }), // 3
    db.accountPlan.create({ data: { name: 'Software e Licenças', type: 'despesa', tenantId: tenant.id } }), // 4
    db.accountPlan.create({ data: { name: 'Consultoria',         type: 'receita', tenantId: tenant.id } }), // 5
    db.accountPlan.create({ data: { name: 'Marketing',           type: 'despesa', tenantId: tenant.id } }), // 6
    db.accountPlan.create({ data: { name: 'Fornecedores',        type: 'despesa', tenantId: tenant.id } }), // 7
  ...
  // Returns a date N months ago, on a specific day
  const dateAt = (monthsAgo, day) => {
  ...
  for (const acc of accountData) {
    const account = await db.account.create({
      data: {
        ...
        status:         acc.status,
        type:           acc.type,
        clientId:       acc.clientId !== null && acc.clientId !== undefined ? clients[acc.clientId].id : null,
  ...
  // ============ COMMISSIONS ============
  // Map some paid accounts to commissions using stable indices
  const pagasReceber = accounts.filter(a => a.type === 'receber' && a.status === 'paga')
  const pendentesReceber = accounts.filter(a => a.type === 'receber' && a.status === 'pendente')

  const commissions = [
    { userId: users[1].id, acc: pagasReceber[0],   amount: Math.round(pagasReceber[0]?.amount  * 0.10), percentage: 10, status: 'paga'     },
    { userId: users[2].id, acc: pagasReceber[1],   amount: Math.round(pagasReceber[1]?.amount  * 0.10), percentage: 10, status: 'paga'     },
    { userId: users[1].id, acc: pagasReceber[2],   amount: Math.round(pagasReceber[2]?.amount  * 0.15), percentage: 15, status: 'paga'     },
    { userId: users[2].id, acc: pagasReceber[3],   amount: Math.round(pagasReceber[3]?.amount  * 0.15), percentage: 15, status: 'paga'     },
    { userId: users[1].id, acc: pagasReceber[4],   amount: Math.round(pagasReceber[4]?.amount  * 0.10), percentage: 10, status: 'paga'     },
    { userId: users[2].id, acc: pagasReceber[5],   amount: Math.round(pagasReceber[5]?.amount  * 0.10), percentage: 10, status: 'paga'     },
    { userId: users[1].id, acc: pagasReceber[6],   amount: Math.round(pagasReceber[6]?.amount  * 0.15), percentage: 15, status: 'paga'     },
    { userId: users[2].id, acc: pagasReceber[7],   amount: Math.round(pagasReceber[7]?.amount  * 0.10), percentage: 10, status: 'paga'     },
    { userId: users[1].id, acc: pagasReceber[8],   amount: Math.round(pagasReceber[8]?.amount  * 0.15), percentage: 15, status: 'paga'     },
    { userId: users[2].id, acc: pagasReceber[9],   amount: Math.round(pagasReceber[9]?.amount  * 0.10), percentage: 10, status: 'paga'     },
    { userId: users[1].id, acc: pendentesReceber[0], amount: Math.round(pendentesReceber[0]?.amount * 0.10), percentage: 10, status: 'pendente' },
    { userId: users[2].id, acc: pendentesReceber[1], amount: Math.round(pendentesReceber[1]?.amount * 0.10), percentage: 10, status: 'pendente' },
  ]
  ...
  // ============ NOTIFICATIONS ============
  await Promise.all([
    db.notification.create({ data: { userId: users[0].id, type: 'vencimento', title: 'Contas Vencidas',        message: 'Existem 3 contas vencidas que precisam de atenção.',          link: '/financeiro' } }),
    db.notification.create({ data: { userId: users[0].id, type: 'pagamento',  title: 'Pagamento Recebido',     message: 'Projeto E-commerce – Comércio ABC está em aberto.' } }),
    ...
    db.notification.create({ data: { userId: users[1].id, type: 'comissao',   title: 'Comissão Disponível',    message: 'Você tem comissões pendentes para revisar.' } }),
    db.notification.create({ data: { userId: users[0].id, type: 'sistema',    title: 'Relatório Mensal',       message: 'O relatório de abril já está disponível.' } }),
    db.notification.create({ data: { userId: users[2].id, type: 'pagamento',  title: 'Meta Batida',            message: 'Abril foi o melhor mês do ano até agora!' } }),
  ])
  ...
  const totalAccounts = accountData.length + 1 + 6 // base + pai parcelamento + 6 parcelas
  console.log('✅ Seed completed successfully!')
  console.log(`   Tenant:          ${tenant.name}`)
  console.log(`   Users:           ${users.length}`)
  console.log(`   Clients:         ${clients.length}`)
  console.log(`   Account Plans:   ${accountPlans.length}`)
  console.log(`   Accounts:        ~${totalAccounts} (13 meses de dados)`)
  console.log(`   Tags:            5`)
